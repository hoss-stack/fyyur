...
class Show(db.Model):
    ...
    def get_upcoming_shows():
      ...
      upcoming_shows_json_list = []
      for show in shows:
        if show.start_time > datetime.now():
          upcoming_shows_json_list.append(
            {
      ...
      return upcoming_shows_json_list
...
class Venue(db.Model):
    ...
    seeking_description = db.Column(db.String(5000), nullable=True, default='We are looking for an exciting artist to perform here!')
    image_link = db.Column(db.String(500))
    # define model relationships
    shows = db.relationship('Show', backref='venue', lazy=True,cascade = "all, delete, delete-orphan")
    
    def as_dict(self):
      return {v.name: getattr(self, v.name) for v in self.__table__.columns}

    @property
    def venue_state_and_city(self):
...
class Artist(db.Model):
    ...
    seeking_description = db.Column(db.String(5000), nullable=True, default='We are looking to perform at an exciting venue!')
    image_link = db.Column(db.String(500))
    # define model relationships
...
def venues():
  try:
    areas = [v.venue_state_and_city for v in Venue.query.distinct(Venue.city, Venue.state).all()]
    for area in areas:
      area['venues'] = [{'name': v.name, 'id': v.id} for v in Venue.query.filter_by(city=area['city'], state=area['state']).all()]
...
def show_venue(venue_id):
  ...
  try:
    ...
    return render_template('pages/show_venue.html', venue=venue.full_details)
  except:
...
def artists():
  try:
    artists = [{'name': a.name, 'id': a.id} for a in Artist.query.all()]
...
def show_artist(artist_id):
  ...
  try:
    ...
    return render_template('pages/show_artist.html', artist=artist.full_details)
  except:
...
def edit_artist(artist_id):
  form = ArtistForm()
  artist = Artist.query.filter_by(id=artist_id).all()[0].full_details
...
def edit_venue(venue_id):
  form = VenueForm()
  try:
    venue = Venue.query.filter_by(id=venue_id).all()[0].full_details
...
def create_show_submission():
  ...
  try:
    ...
    if len(Artist.query.filter_by(id=artist_id).all()) == 0 or len(Venue.query.filter_by(id=venue_id).all()) == 0:
      raise ValueError('Either venue_id or artist_id is not found on the databse')
    if datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S') < datetime.now():
      raise ValueError('Show time has already passed')
